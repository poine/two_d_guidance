...
class ImgPublisher(trr_rpu.DebugImgPublisher):
    def __init__(self, path, img_topic):
        ...
        lm_start_idx, lm_start_point   = path.find_point_at_dist_from_idx(0, _d=path.lm_s[path.LM_START])
        lm_finish_idx, lm_finish_point = path.find_point_at_dist_from_idx(0, _d=path.lm_s[path.LM_FINISH])
