...
class Guidance:
    ...
    def __init__(self, lookahead=0.4, vel_sp=0.2, path_fname=None):
        if path_fname is not None: self.path = trr_u.TrrPath(path_fname)
        else: self.path = None
    ...
    def compute(self, lane_model, s=float('inf'), expl_noise=0.025, dy=0., avoid_obstacles=False):
        lin = self.vel_ctl.get(lane_model)
        _is, _ = self.path.find_point_at_dist_from_idx(0, _d=s)
        #print s, _is
        lin =  self.path.vels[_is]
        if avoid_obstacles:
